...
TUNNEL_TCP_PORT=$(get_variable "TUNNEL_TCP_PORT")
...
if [ -n "$CLOUDFLARE_AUTH_EMAIL" ] && [ -n "$CLOUDFLARE_API_KEY" ] && [ -n "$CLOUDFLARE_ZONE_ID" ]; then
    ...
    echo "ℹ️ API Key Auth Test Result: $cf_auth_test"
...
if [ -n "$CLOUDFLARE_API_TOKEN" ]; then
    ...
    echo "ℹ️ API Token Auth Test Result: $cf_token_test"
...
if [ "$TARGET_SERVICE_AVAILABLE" != "true" ]; then
    echo "⚠️ WARNING: No service detected on $TARGET_HOST:$NGROK_TCP_PORT after $PORT_RETRY_COUNT attempts."
...
echo "ℹ️ Using CNAME record name: $CLOUDFLARE_CNAME_RECORD_NAME"
echo "ℹ️ Using SRV record: $CLOUDFLARE_SRV_RECORD"
...
if [[ "$CLOUDFLARE_SRV_RECORD" =~ ^(_[^.]+)\.(_[^.]+)\.(.*) ]]; then
    ...
    echo "⚠️ WARNING: SRV record doesn't match the expected format (_service._proto.domain.tld)"
    echo "Falling back to default (_minecraft._tcp.example.com). SRV record may not work correctly."
...
echo "ℹ️ CNAME lookup response: $cname_record_response"

cname_record_exists=false
cname_record_id=""

if [[ $cname_record_response == *"\"count\":0"* ]]; then
    echo "ℹ️ CNAME record does not exist in Cloudflare. Creating it now..."
    ...
    echo "ℹ️ CNAME creation response: $create_cname_response"
    ...
    if [[ $create_cname_response == *"\"success\":true"* ]]; then
        ...
        echo "ℹ️ Created CNAME record ID: $cname_record_id"
    else
...
if [ -n "$CLOUDFLARE_SRV_RECORD" ] && [ "$CLOUDFLARE_SRV_RECORD" != "_minecraft._tcp.example.com" ]; then
    ...
    echo "ℹ️ Looking up SRV record with name: $CLOUDFLARE_SRV_RECORD"
    ...
    echo "ℹ️ SRV lookup response: $srv_record_response"

    if [[ $srv_record_response == *"\"count\":0"* ]]; then
        echo "ℹ️ SRV record does not exist in Cloudflare. Creating it now..."
        ...
        echo "ℹ️ SRV creation response: $create_srv_response"
        ...
        if [[ $create_srv_response == *"\"success\":true"* ]]; then
            ...
            echo "ℹ️ Created SRV record ID: $srv_record_id"
        else
...
update_records() {
    ...
    if [ "$cname_record_exists" = true ]; then
        echo "ℹ️ Updating CNAME record in Cloudflare..."
        if [ "$CLOUDFLARE_AUTH_METHOD" = "API_KEY" ]; then
            update_cname_response=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$cname_record_id" \
                ...
                -H "X-Auth-Key: $CLOUDFLARE_API_KEY" \
                -H "Content-Type: application/json" \
                --data "{\"type\":\"CNAME\",\"name\":\"$CLOUDFLARE_CNAME_RECORD_NAME\",\"content\":\"$tunnel_host\"}")
        elif [ "$CLOUDFLARE_AUTH_METHOD" = "API_TOKEN" ]; then
            update_cname_response=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$cname_record_id" \
                -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
                -H "Content-Type: application/json" \
    ...
    if [ "$srv_record_exists" = true ]; then
        echo "ℹ️ Updating SRV record in Cloudflare..."
        echo "ℹ️ Using service: $SRV_SERVICE, proto: $SRV_PROTO for SRV record"
        ...
        if [ "$CLOUDFLARE_AUTH_METHOD" = "API_KEY" ]; then
            update_srv_response=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$srv_record_id" \
                ...
                -H "X-Auth-Key: $CLOUDFLARE_API_KEY" \
                -H "Content-Type: application/json" \
                --data "{\"type\":\"SRV\",\"name\":\"$CLOUDFLARE_SRV_RECORD\",\"data\": {\"service\":\"$SRV_SERVICE\",\"proto\":\"$SRV_PROTO\",\"name\":\"$SRV_DOMAIN\",\"priority\":1,\"weight\":1,\"port\":$tunnel_port,\"target\":\"$CLOUDFLARE_CNAME_RECORD_NAME\"}}")
        elif [ "$CLOUDFLARE_AUTH_METHOD" = "API_TOKEN" ]; then
            update_srv_response=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records/$srv_record_id" \
                -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" \
                -H "Content-Type: application/json" \
    ...
    echo "✅ Done! You can connect to your server using $tunnel_host:$tunnel_port"
    echo "ℹ️ CNAME record: $CLOUDFLARE_CNAME_RECORD_NAME -> $tunnel_host"
    if [ "$srv_record_exists" = true ]; then
        echo "ℹ️ SRV record: $CLOUDFLARE_SRV_RECORD -> $CLOUDFLARE_CNAME_RECORD_NAME:$tunnel_port"
    fi
}
# === END OF COMMON SETUP ===
...
start_ngrok() {
    echo "ℹ️ [NGROK] starting..."

    ngrok config add-authtoken $NGROK_AUTH_TOKEN

    echo "ℹ️ Running: ngrok tcp $TARGET_HOST:$NGROK_TCP_PORT"
    ...
    echo "ℹ️ Ngrok Host is $ngrok_host"
    echo "ℹ️ Ngrok Port is $ngrok_port"

    update_records "$ngrok_host" "$ngrok_port"
}

start_pinggy() {
    echo "ℹ️ [PINGGY] starting..."
    # TODO: Make Pinggy implementation
}
# === END OF TOOL-SPECIFIC FUNCTIONS ===

case "$service_provider" in
    ngrok) start_ngrok "$@" ;;
    pinggy) start_pinggy "$@" ;;
    *) echo "❌ Unknown option: $service_provider" >&2; exit 1 ;;
esac
